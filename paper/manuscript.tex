...
\section{Methods}
The selective iterative latent variable refinement (SILVR) method makes use of the pre-trained equivariant diffusion model (EDM) as introduced by Hoogeboom \textit{et al.}~\cite{hoogeboom2022equivariant}. The core of the method is the introduction of an additional refinement within the denoising process during run time. The resulting SILVR model can then produce conditional samples without any conditional training. In this work, we show how SILVR can be applied to fragment-based drug design to produce molecules resembling a reference set of atomic coordinates. 
