...
\end{equation}
We use the notation $\kappa$ to generate a sample [\mathbf{\hat{x}}, \mathbf{\hat{h}}] from the EDM. 
...
Similarly to Choi \texit{et al.}, we update the diffusion process at noise level $t$ in the latent space $\mathbf{z}$, with the reference $\mathbf{\tilde{z}}$ using a factor, or vector if used at variable rates for different atoms $r_{\mathrm{S}}$. We call $r_{\mathrm{S}}$ the SILVR rate and this leads to an overall update or conditioning towards a reference at each step in the generative denoising process according to the SILVR equation~\ref{eqn:SILVR}.
\begin{equation}
