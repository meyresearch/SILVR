...
def main(argv=[__name__]):
    ...
    for mcmol in ifs.GetOEMols():
        print("cleaning",mcmol.GetTitle())
        
        if oechem.OEDetermineComponents(mcmol)[0] > 1:
            print("Molecule is fragmented. Skipping.")
            break
        
        oechem.OEAssignImplicitHydrogens(mcmol)
        oequacpac.OERemoveFormalCharge(mcmol)
        oequacpac.OEHypervalentNormalization(mcmol)
        oechem.OEAddExplicitHydrogens(mcmol)
        oequacpac.OESetNeutralpHModel(mcmol)        
